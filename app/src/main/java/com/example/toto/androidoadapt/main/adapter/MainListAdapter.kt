 ...
 *
 */
class MainListAdapter(var context: Context, var list: List<String>, var RecyclerItemlistener: (RecyclerView.ViewHolder, Int) -> Unit) : RecyclerView.Adapter<MainListAdapter.MainListHolder>() {

    override fun onBindViewHolder(holder: MainListHolder, position: Int) {
        holder.onBindViewHolder(list[position])
        holder.itemView.setOnClickListener {
            RecyclerItemlistener(holder, position)
        }
    }
    ...
    class MainListHolder(itemView: View) : TCommonHolder(itemView), View.OnClickListener {
        var mContainer: ConstraintLayout = findView(R.id.main_list_layout);
